...
import { useMemoryCardEditor, type DisplayImage } from '../hooks/useMemoryCardEditor';
...
const noopRemoveImage = (_image: DisplayImage) => undefined;

const MemoryCardFrame: React.FC<MemoryCardFrameProps> = ({ isHer, children, footer }) => (
  <div
...
const ReadOnlyMemoryCard: React.FC<MemoryCardProps & { onStartEdit: () => void }> = ({
  memory,
  onDelete,
  currentUser,
  onStartEdit,
    ...
    <MemoryCardFrame
      ...
      footer={(
        <MemoryCardFooter
          ...
          onCancelEdit={onStartEdit}
          onSaveEdit={onStartEdit}
...
const EditableMemoryCard: React.FC<MemoryCardProps & { onExitEdit: () => void }> = ({
  memory,
  onDelete,
  onUpdate,
  currentUser,
  ...
  const {
    ...
    handleCancelEdit,
    handleSaveEdit,
  } = useMemoryCardEditor(memory, onUpdate, true);
  ...
  return (
    ...
    >
      ...
      {(editImageUrls.length + editImageFiles.length) < MAX_MEMORY_IMAGES && (
        <button
          onClick={(e) => {
            e.stopPropagation();
...
export const MemoryCard: React.FC<MemoryCardProps> = React.memo((props) => {
  const [editing, setEditing] = useState(false);

  if (editing) {
    return <EditableMemoryCard {...props} onExitEdit={() => setEditing(false)} />;
  }

  return <ReadOnlyMemoryCard {...props} onStartEdit={() => setEditing(true)} />;
}, (prevProps, nextProps) => {
  return (
    prevProps.memory.id === nextProps.memory.id &&
    prevProps.memory.content === nextProps.memory.content &&
    prevProps.memory.createdAt === nextProps.memory.createdAt &&
    prevProps.memory.author === nextProps.memory.author &&
